...
public class Airplain : MonoBehaviour
{
    ...
    private void Update()
    {
        if (teamIndex != -2 && GetComponent<Move>().target == Vector3.zero) {
            ...
            {
                planet.GetComponent<Teams>().teams[partOfPlanet.teamIndex].TakeDamage(planet.GetComponent<Teams>().bombDamage, partOfPlanet, teamIndex);
            }
